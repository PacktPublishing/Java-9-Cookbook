...
public class Chapter05Streams {
    ...
    public static void demo1_createCollections() {
        ...
        System.out.println();
        //Compare with previous (not immutable) Set/Map examples
        Set<String> s = new HashSet<>(Arrays.asList("This ", "is ", "mutable ", "set ", "new HashSet(Arrays.asList()) "));
        s.add("added ");
    ...
    public static void demo2_createStreams() {
        ...
        Path dir = FileSystems.getDefault().getPath("src/com/packt/cookbook/ch05_streams/");
        ...
        System.out.println();
        Path f = FileSystems.getDefault().getPath("src/com/packt/cookbook/ch05_streams/");
        BiPredicate<Path, BasicFileAttributes> select = (p, b) -> p.getFileName().toString().contains("Factory");
        try(Stream<Path> stream = Files.find(f, 2, select, FileVisitOption.FOLLOW_LINKS)){
